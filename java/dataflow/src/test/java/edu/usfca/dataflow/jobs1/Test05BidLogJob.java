...
//  public Timeout timeout = Timeout.millis(5000);
  ...
  public void testSuspiciousUsers1() {
    ...
    PAssert.that(
        getSuspiciousIDs(dps, aps,
            4, 3, 8, 10)
        .apply(MapElements.into(TypeDescriptor.of(DeviceId.class))
            .via((DeviceId id) -> id.toBuilder().setUuid(id.getUuid().toLowerCase()).build()))) // PC<DeviceId>
