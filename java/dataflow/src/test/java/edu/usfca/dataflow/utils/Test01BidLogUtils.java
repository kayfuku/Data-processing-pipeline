 ...
 */
public class Test01BidLogUtils {
    ...
    // Test cases with valid samples.
    {
      BidLog.Builder bl = BidLog.newBuilder().setExchange(Exchange.OPENX).setBidResult(BidResult.BID).setBidPrice(123)
          ...
          .setBidRequest(br = BidRequest.newBuilder().setId("bidid")
              .setDevice(device = Device.newBuilder().setOs("Android").setIfa(TestBase.UUID1)
                  .setGeo(Geo.newBuilder().setCountry("usa").setRegion("ca")))
              .setApp(app = App.newBuilder().setBundle(TestBase.Bundle1)));
      assertTrue(BidLogUtils.isValid(valid1 = bl.build()));
      bl.setBidResult(BidResult.NO_CANDIDATE).setBidPrice(0);
  ...
  public void testIsValid03() {
    ...
    // Test cases with valid samples.
    {
      app = App.newBuilder().setBundle(TestBase.Bundle1).build();
      geo = Geo.newBuilder().setCountry("usa").setRegion("ca").build();
      device = Device.newBuilder().setOs("Android").setIfa(TestBase.UUID1).setGeo(geo).build();
  ...
  public void testGetDeviceProfile() {
    ...
    // check dp1.
    {
      ...
      assertEquals(TestBase.UUID1.toLowerCase(), dp1.getDeviceId().getUuid().toLowerCase());
      assertEquals(1, dp1.getAppCount());
      assertEquals(TestBase.Bundle1, dp1.getApp(0).getBundle());
    ...
    // check dp2.
    {
      ...
      assertEquals(TestBase.UUID1.toLowerCase(), dp2.getDeviceId().getUuid().toLowerCase());
      assertEquals(1, dp2.getAppCount());
      assertEquals(TestBase.Bundle1, dp2.getApp(0).getBundle());
